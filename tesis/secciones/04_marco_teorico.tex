...
Un modelo con $L$ capas y $h$ cabezas produce $L \times h$ matrices de atención, de modo que hace falta un procedimiento para obtener una sola que represente al modelo completo. El framework no fija cuál debe ser ese procedimiento, sino las condiciones que su resultado tiene que cumplir, según se detalla en el capítulo \ref{cap:propuesta}. La instancia de este trabajo emplea el attention rollout de \textcite{abnar2020quantifying}, que propaga la atención a través de las capas incorporando las conexiones residuales:
...
con $Z$ el espacio interno aprendido por la red en una capa $L$. A partir de esa representación se construye la matriz de interacción $A \in \mathbb{R}^{n\times n}$, que es la matriz agregada de la sección anterior y no una de las matrices de atención por capa y cabeza de las que proviene, y donde cada elemento $A_{ij}$ expresa la intensidad de interacción entre dos componentes latentes $i$ y $j$. La matriz es agnóstica al dominio: el índice $n$ puede recorrer índices espectrales, sensores industriales o variables de proceso sin que cambie nada de lo anterior.
...
\section{Medición de la calidad de una explicación}
